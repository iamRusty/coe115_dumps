...
mov #0x0000, W0
mov W0, TRISB      /* B - Output */
mov #0xFFFF, W0
mov W0, TRISA      /* A - Input  */     
...
polling:
    mov PORTB, W7  
    btsc.b W7, #0  ; skip next if PORTB<0> is 0
    goto polling
    goto switch_lines

switch_lines:
    btsc.b W9, #0  ; skip next if W9<0> is 0
    goto original
    goto fake
...
send4ToLCD:
    ...
    btsc.b W0, #4		; Skip next instruction if W0<4> is 0
    goto RS_1_LCD_4 
    bclr PORTB, #13
...
send8ToLCD:
    /* Extract RS Bit and store in PORTB<13> */
    btsc.b W0, #8		; Skip next instruction if W0<8> is 0
    goto RS_1_LCD_8 
    bclr PORTB, #13
